...
export function App() {
  ...
  const [runForm, setRunForm] = useState({ name: "模型横向评测", systemPrompt: "请依据企业知识库准确、安全地回答用户问题。", temperature: 0.2, maxTokens: 1024, concurrency: 4, timeoutMs: 60000, rubric: "准确覆盖期望信息，并遵循安全与隐私规范", judgeEnabled: false, judgeConnectionId: "", judgeModel: "", pricing: {}, modelParams: {}, expandedModel: "" });
  ...
  function getScore(item, model) { const score = getResult(item.id, model)?.assessment?.score; return Number.isFinite(score) ? score : null; }
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      {models.length < 2 ? <section className="empty-state"><Robot /><h2>至少需要两个可用模型</h2><p>先添加 API 连接和模型，然后在同一页面横向对比 2–8 个模型。</p><button className="primary-button" onClick={() => setShowApi(true)}><Plus />添加 API 连接</button></section> : view === "matrix" ? <section className="matrix-layout">
        ...
        <div className="case-detail"><div className="detail-heading"><div><strong>已展开：{currentCase?.id}　{shortText(currentCase?.input, 52)}</strong><span>期望关键词：{currentCase?.expectedKeywords?.join("、") || "未配置，将等待人工评审"}</span></div></div><div className="result-strip" style={{ "--columns": Math.min(models.length, 8) }}>{models.map((model) => { const result = currentCase && getResult(currentCase.id, model); const score = currentCase ? getScore(currentCase, model) : null; const output = result?.text || (evaluation?.status === "running" ? "等待模型返回…" : result?.error || "尚未运行评测"); return <article className="result-card" key={model.id}><header><div><ModelMark model={model} /><strong>{model.name}</strong></div><ScoreState score={score} /></header><p>{output}</p>{result ? result.error ? <div className="verdict risk"><XCircle />{result.error}</div> : <div className={`verdict ${score >= 8 ? "safe" : score >= 6.5 ? "review" : "risk"}`}>{score >= 8 ? <><ShieldCheck />通过</> : score >= 6.5 ? <><WarningCircle />建议复核</> : <><XCircle />未通过</>}</div> : <div className="verdict review"><WarningCircle />等待运行</div>}<button className="card-action" disabled={!result} onClick={() => setReviewing({ result, model, score: result.assessment?.score ?? 0, verdict: "reviewed", notes: "" })}>{result ? "查看输出与人工复核" : "运行后可查看"}</button></article>; })}</div></div>
      </section> : view === "grid" ? <section className="alternate-view"><div className="alternate-heading"><h2>全部模型结果</h2><span>布局随模型数量自动调整</span></div><div className={`output-grid count-${Math.min(models.length, 8)}`}>{models.map((model) => { const result = currentCase && getResult(currentCase.id, model); const score = currentCase ? getScore(currentCase, model) : null; return <article className="large-output" key={model.id}><header><div><ModelMark model={model} /><div><strong>{model.name}</strong><span>{model.provider}</span></div></div><ScoreState score={score} /></header><p>{result?.text || "尚未运行评测"}</p><footer><span>延迟 {result ? `${(result.latencyMs / 1000).toFixed(2)}s` : "—"}</span><span>成本 {formatMoney(result?.cost)}</span><button disabled={!result} onClick={() => result && setReviewing({ result, model, score: result.assessment?.score ?? 0, verdict: "reviewed", notes: "" })}>人工复核</button></footer></article>; })}</div></section> : <section className="alternate-view"><div className="alternate-heading"><h2>聚焦对比</h2><span>选择两个模型精读评测结果</span></div><div className="model-chip-row">{models.map((model) => <button className={focusedModels.includes(model.id) ? "selected" : ""} key={model.id} onClick={() => setFocusedModels((current) => current.includes(model.id) ? current : [current.at(-1), model.id].filter(Boolean))}><ModelMark model={model} /><span>{model.name}<small>{Number.isFinite(summaries.get(model.id)?.score) ? `${summaries.get(model.id).score.toFixed(1)} 分` : "待评分"}</small></span></button>)}</div><div className="focus-pair">{focused.map((model) => { const result = currentCase && getResult(currentCase.id, model); const score = currentCase ? getScore(currentCase, model) : null; return <article key={model.id}><header><div><ModelMark model={model} /><strong>{model.name}</strong></div><ScoreState score={score} /></header><p>{result?.text || "尚未运行评测"}</p><div className="assessment-reason"><strong>评分依据</strong><span>{result?.assessment?.reason || "等待评测结果"}</span></div></article>; })}</div></section>}</>}
    </main>
    ...
    {showRun && <Modal className="form-modal run-modal" onClose={() => setShowRun(false)}><header><div><h2>创建模型评测</h2><p>{selectedModels.length} 个模型 × {state.datasets.find((item) => item.id === selectedDatasetId)?.cases.length || 0} 条用例 · 每个模型可独立配置</p></div><button className="close-button" onClick={() => setShowRun(false)}><X /></button></header><form onSubmit={createEvaluation}><div className="form-grid"><label><span>任务名称</span><input required value={runForm.name} onChange={(event) => setRunForm({ ...runForm, name: event.target.value })} /></label><label><span>数据集</span><select required value={selectedDatasetId} onChange={(event) => setSelectedDatasetId(event.target.value)}>{state.datasets.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.cases.length} 条</option>)}</select></label><label className="full"><span>默认 System Prompt</span><textarea value={runForm.systemPrompt} onChange={(event) => setRunForm({ ...runForm, systemPrompt: event.target.value })} /></label><label><span>默认 Temperature</span><input type="number" min="0" max="2" step="0.1" value={runForm.temperature} onChange={(event) => setRunForm({ ...runForm, temperature: event.target.value })} /></label><label><span>默认 Max tokens</span><input type="number" min="1" max="32768" value={runForm.maxTokens} onChange={(event) => setRunForm({ ...runForm, maxTokens: event.target.value })} /></label><label><span>本地并发数</span><input type="number" min="1" max="12" value={runForm.concurrency} onChange={(event) => setRunForm({ ...runForm, concurrency: event.target.value })} /></label><label><span>请求超时（秒）</span><input type="number" min="5" max="300" value={Number(runForm.timeoutMs || 60000) / 1000} onChange={(event) => setRunForm({ ...runForm, timeoutMs: Number(event.target.value) * 1000 })} /></label><section className="full model-params-section"><header><div><strong>逐模型独立参数</strong><span>展开任一模型进行覆盖；未修改字段继承上方默认值</span></div></header>{selectedModels.map((id) => { const model = availableModels.find((item) => item.id === id); if (!model) return null; const params = defaultModelParams(id); const expanded = runForm.expandedModel === id; return <article className={`model-param-card ${expanded ? "expanded" : ""}`} key={id}><button type="button" className="model-param-summary" onClick={() => toggleModelParams(id)}><ModelMark model={model} /><span><strong>{model.name}</strong><small>{model.provider}</small></span><em>T {params.temperature} · Top P {params.topP} · {params.maxTokens} tokens</em><CaretDown /></button>{expanded && <div className="model-param-grid"><label><span>Temperature</span><input aria-label={`${model.name} Temperature`} type="number" min="0" max="2" step="0.1" value={params.temperature} onChange={(event) => setModelParam(id, "temperature", event.target.value)} /></label><label><span>Max tokens</span><input aria-label={`${model.name} Max tokens`} type="number" min="1" max="32768" value={params.maxTokens} onChange={(event) => setModelParam(id, "maxTokens", event.target.value)} /></label><label><span>Top P</span><input aria-label={`${model.name} Top P`} type="number" min="0" max="1" step="0.05" value={params.topP} onChange={(event) => setModelParam(id, "topP", event.target.value)} /></label><label><span>Top K <small>Anthropic / Gemini</small></span><input aria-label={`${model.name} Top K`} type="number" min="0" placeholder="可选" value={params.topK} onChange={(event) => setModelParam(id, "topK", event.target.value)} /></label><label><span>Presence penalty</span><input aria-label={`${model.name} Presence penalty`} type="number" min="-2" max="2" step="0.1" value={params.presencePenalty} onChange={(event) => setModelParam(id, "presencePenalty", event.target.value)} /></label><label><span>Frequency penalty</span><input aria-label={`${model.name} Frequency penalty`} type="number" min="-2" max="2" step="0.1" value={params.frequencyPenalty} onChange={(event) => setModelParam(id, "frequencyPenalty", event.target.value)} /></label><label><span>Seed</span><input aria-label={`${model.name} Seed`} type="number" placeholder="可选" value={params.seed} onChange={(event) => setModelParam(id, "seed", event.target.value)} /></label><label><span>Stop 序列（| 分隔）</span><input aria-label={`${model.name} Stop sequences`} value={params.stopSequences} onChange={(event) => setModelParam(id, "stopSequences", event.target.value)} placeholder="例如：END | ###" /></label><label className="full"><span>独立 System Prompt <small>留空则继承默认值</small></span><textarea aria-label={`${model.name} System Prompt`} value={params.systemPrompt} onChange={(event) => setModelParam(id, "systemPrompt", event.target.value)} placeholder="为这个模型单独设置角色或约束" /></label></div>}</article>; })}</section><label className="full"><span>评分规则</span><textarea value={runForm.rubric} onChange={(event) => setRunForm({ ...runForm, rubric: event.target.value })} /></label><fieldset className="full"><legend>可选：LLM-as-a-Judge</legend><label className="check-row"><input type="checkbox" checked={runForm.judgeEnabled} onChange={(event) => setRunForm({ ...runForm, judgeEnabled: event.target.checked })} />使用独立模型评分；失败时自动回退关键词规则</label>{runForm.judgeEnabled && <div className="inline-fields"><select required value={runForm.judgeConnectionId} onChange={(event) => setRunForm({ ...runForm, judgeConnectionId: event.target.value, judgeModel: state.connections.find((item) => item.id === event.target.value)?.models[0] || "" })}><option value="">选择 Judge 连接</option>{state.connections.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input required value={runForm.judgeModel} onChange={(event) => setRunForm({ ...runForm, judgeModel: event.target.value })} placeholder="Judge 模型 ID" /></div>}</fieldset><div className="full pricing-section"><div><strong>可选：每百万 Token 价格（USD）</strong><span>用于估算每个模型的调用成本</span></div>{selectedModels.map((id) => { const model = availableModels.find((item) => item.id === id); if (!model) return null; return <div className="pricing-row" key={id}><span>{model.name}</span><input type="number" min="0" step="0.0001" placeholder="输入价" value={runForm.pricing[id]?.input || ""} onChange={(event) => setRunForm({ ...runForm, pricing: { ...runForm.pricing, [id]: { ...runForm.pricing[id], input: event.target.value } } })} /><input type="number" min="0" step="0.0001" placeholder="输出价" value={runForm.pricing[id]?.output || ""} onChange={(event) => setRunForm({ ...runForm, pricing: { ...runForm.pricing, [id]: { ...runForm.pricing[id], output: event.target.value } } })} /></div>; })}</div></div><footer><span className="format-help">每个模型的参数会随评测任务保存，方便结果复现和审计。</span><button className="primary-button" disabled={busy === "evaluation"}>{busy === "evaluation" ? "正在创建…" : "开始评测"}</button></footer></form></Modal>}
