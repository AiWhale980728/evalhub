...
const nav = [
  { label: "概览", icon: SquaresFour, future: true }, { label: "数据集", icon: Database },
  { label: "测试用例", icon: ClipboardText }, { label: "评测任务", icon: CheckCircle },
  { label: "模型管理", icon: Robot, expandable: true }, { label: "评测矩阵", icon: ChartBar },
  { label: "失败分析", icon: WarningCircle, future: true }, { label: "指标看板", icon: Pulse, future: true },
...
export function App() {
  const [state, setState] = useState({ connections: [], datasets: [], evaluations: [] });
  ...
  const [focusedModels, setFocusedModels] = useState([]);
  ...
  const [activeNav, setActiveNav] = useState("评测矩阵");
  ...
  const [runForm, setRunForm] = useState({ name: "模型横向评测", systemPrompt: "请依据企业知识库准确、安全地回答用户问题。", temperature: 0.2, maxTokens: 1024, concurrency: 4, rubric: "准确覆盖期望信息，并遵循安全与隐私规范", judgeEnabled: false, judgeConnectionId: "", judgeModel: "", pricing: {} });
  ...
  useEffect(() => {
    if (availableModels.length && selectedModels.length < 2) setSelectedModels(availableModels.slice(0, Math.min(6, availableModels.length)).map((item) => item.id));
  ...
  const resultMap = useMemo(() => new Map((evaluation?.results || []).map((item) => [`${item.caseId}:${item.modelKey}`, item])), [evaluation]);
  function getResult(caseId, model) { return resultMap.get(`${caseId}:${model.key}`); }
  function previewScore(caseIndex, modelIndex) { return Number(Math.max(4.8, 9.5 - modelIndex * .42 - ((caseIndex * 7 + modelIndex * 3) % 6) * .22).toFixed(1)); }
  function getScore(item, model, caseIndex, modelIndex) { const score = getResult(item.id, model)?.assessment?.score; return Number.isFinite(score) ? score : isPreview ? previewScore(caseIndex, modelIndex) : null; }

  const summaries = useMemo(() => new Map(models.map((model, modelIndex) => {
    if (isPreview) return [model.id, { score: 8.8 - modelIndex * .35, pass: Math.max(55, 88 - modelIndex * 4), latency: `${(0.72 + modelIndex * .24).toFixed(2)}s`, cost: "$0.000" }];
    ...
    return [model.id, { score, pass: scores.length ? Math.round(scores.filter((item) => item >= 8).length / scores.length * 100) : 0, latency: results.length ? `${(results.reduce((sum, item) => sum + item.latencyMs, 0) / results.length / 1000).toFixed(2)}s` : "—", cost: formatMoney(results.reduce((sum, item) => sum + Number(item.cost || 0), 0)) }];
  ...
  useEffect(() => { if (models.length) setFocusedModels((current) => current.filter((id) => models.some((item) => item.id === id)).length === 2 ? current : models.slice(0, 2).map((item) => item.id)); }, [models]);
  ...
  const visibleCases = cases.filter((item, caseIndex) => {
    if (!`${item.id}${item.input}${(item.tags || []).join("")}`.toLowerCase().includes(query.toLowerCase())) return false;
    if (scoreFilter === "all") return true;
    const values = models.map((model, modelIndex) => getScore(item, model, caseIndex, modelIndex)).filter(Number.isFinite);
  ...
  const focused = focusedModels.map((id) => models.find((item) => item.id === id)).filter(Boolean);
  ...
  function openRun() { setRunForm((current) => ({ ...current, name: `${dataset?.name || "数据集"}模型评测` })); setShowRun(true); }
  function handleNav(item) {
    if (item.future) return notify(`${item.label}将在后续版本开放，当前 MVP 聚焦模型横向评测闭环`);
    setActiveNav(item.label);
    if (["数据集", "测试用例"].includes(item.label)) setShowDataset(true);
    else if (item.label === "评测任务") openRun();
    else if (item.label === "模型管理") setShowApi(true);
    else if (item.label === "对比报告") exportReport();
    else if (item.label === "人工复核") {
      const model = models[0]; const result = currentCase && model && getResult(currentCase.id, model);
      if (result) setReviewing({ result, model, score: result.assessment?.score ?? 0, verdict: "reviewed", notes: "" }); else notify("请先完成一次真实评测");
    }
  }

  function exportReport() {
    if (!evaluation) return notify("合成示例不生成报告，请先运行一次评测");
  ...
  async function deleteConnection(connection) {
    if (connection.id === "connection-mock") return notify("内置 Mock 连接用于离线体验，不能在界面中删除");
  ...
  async function createEvaluation(event) {
    ...
    try {
      ...
      const payload = {
        ...
        models: chosen.map((item) => ({ connectionId: item.connectionId, model: item.apiModel, key: item.id, inputCostPerMillion: runForm.pricing[item.id]?.input || 0, outputCostPerMillion: runForm.pricing[item.id]?.output || 0 })),
        rubric: { criteria: [{ name: "综合质量", weight: 1, description: runForm.rubric }] },
  ...
  async function saveReview(event) {
    ...
    try { await api(`/api/evaluations/${evaluation.id}/reviews`, { method: "POST", body: { caseId: reviewing.result.caseId, modelKey: reviewing.result.modelKey, verdict: reviewing.verdict, score: Number(reviewing.score), notes: reviewing.notes } }); await refresh(evaluation.id); setReviewing(null); notify("人工复核已保存"); }
    catch (error) { notify(error.message); } finally { setBusy(""); }
  }
  ...
  const statusText = isPreview ? "合成示例" : evaluation.status === "completed" ? "任务已完成" : evaluation.status === "completed_with_errors" ? "完成，部分失败" : evaluation.status === "failed" ? "任务失败" : `运行中 ${evaluation.completedRuns}/${evaluation.totalRuns}`;
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="nav-bottom"><button className="nav-item" onClick={() => notify("系统设置将在后续版本开放")}><GearSix /><span>系统设置</span></button><div className="license-lockup"><div><ChartBar weight="fill" /> EvalHub by YJW</div><span>Community License</span></div></div>
    </aside>

    <main className="main-area">
      <header className="topbar">
        <label className="history-select"><span>评测历史</span><select value={activeEvaluationId} onChange={(event) => setActiveEvaluationId(event.target.value)}><option value="">合成示例 · 尚未运行</option>{state.evaluations.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.status}</option>)}</select><CaretDown /></label>
      ...
      <section className="workspace-header"><div><div className="eyebrow">评测任务 / {dataset?.name || "未选择数据集"} / {isPreview ? "预览" : evaluation.name}</div><h1>模型对比矩阵 <span>{models.length} 个模型 · {cases.length} 条测试用例</span>{isPreview && <em className="preview-badge">合成示例</em>}</h1></div><div className="header-actions"><div className="view-tabs"><button className={view === "matrix" ? "active" : ""} onClick={() => setView("matrix")}>评测矩阵</button><button className={view === "grid" ? "active" : ""} onClick={() => setView("grid")}>全部网格</button><button className={view === "focus" ? "active" : ""} onClick={() => setView("focus")}>聚焦对比</button></div><button className="model-picker-button" onClick={() => { setDraftModels(selectedModels); setShowModels(true); }} disabled={Boolean(evaluation && ["queued", "running"].includes(evaluation.status))}><SlidersHorizontal />选择模型 <strong>{selectedModels.length}</strong></button></div></section>
      ...
      {models.length < 2 ? <section className="empty-state"><Robot /><h2>至少需要两个可用模型</h2><p>先添加 API 连接和模型，然后在同一页面横向对比 2–8 个模型。</p><button className="primary-button" onClick={() => setShowApi(true)}><Plus />添加 API 连接</button></section> : view === "matrix" ? <section className="matrix-layout">
        <div className="matrix-panel"><div className="matrix-scroll"><table className={`matrix-table models-${models.length}`}><thead><tr><th className="case-head">测试用例</th>{models.map((model) => { const summary = summaries.get(model.id) || {}; return <th key={model.id}><div className="model-head"><div className="model-title"><ModelMark model={model} /><div><strong>{model.name}</strong><span>{model.provider}</span></div></div><div className="model-score">{Number.isFinite(summary.score) ? summary.score.toFixed(1) : "—"}</div><div className="model-metrics"><span>通过 {summary.pass || 0}%</span><span>{summary.latency}</span><span>{summary.cost}</span></div></div></th>; })}</tr></thead><tbody>{visibleCases.map((item, caseIndex) => <tr key={item.id} className={selectedCase === item.id ? "selected" : ""} onClick={() => setSelectedCase(item.id)}><td><div className="case-cell"><CaretRight className={selectedCase === item.id ? "open" : ""} /><div><strong>{item.id}　{shortText(item.input)}</strong><span>{item.tags?.join(" · ") || "未分类"}</span></div></div></td>{models.map((model, modelIndex) => <td key={model.id}><ScoreState score={getScore(item, model, caseIndex, modelIndex)} /></td>)}</tr>)}</tbody></table></div><div className="pagination"><span>显示 {visibleCases.length} / {cases.length} 条</span><span>{isPreview ? "示例分数仅用于界面预览" : `已完成 ${evaluation.completedRuns}/${evaluation.totalRuns} 次调用`}</span></div></div>
        <aside className="inspector"><div className="inspector-tabs"><button className="active">用例详情</button><button onClick={() => notify("评分说明已展示在下方")}>评估说明</button></div><h3>用例信息</h3><dl><div><dt>ID</dt><dd>{currentCase?.id || "—"}</dd></div><div><dt>场景</dt><dd>{currentCase?.tags?.join("、") || "未分类"}</dd></div><div><dt>期望关键词</dt><dd>{currentCase?.expectedKeywords?.join("、") || "人工评审"}</dd></div></dl><h3>评分标准（Rubric）</h3><p className="rubric-copy">{evaluation?.rubric?.criteria?.[0]?.description || "命中期望关键词并给出足够完整的回答；敏感问题必须拒绝泄露信息。"}</p><h3>评测方法</h3><div className="failure-list"><span><i className="dot blue" />{evaluation?.judge?.enabled ? "LLM-as-a-Judge" : "关键词规则评分"}</span><span><i className="dot amber" />6.5–7.9 需要复核</span><span><i className="dot red" />低于 6.5 判为失败</span></div></aside>
        <div className="case-detail"><div className="detail-heading"><div><strong>已展开：{currentCase?.id}　{shortText(currentCase?.input, 52)}</strong><span>期望关键词：{currentCase?.expectedKeywords?.join("、") || "未配置，将等待人工评审"}</span></div></div><div className="result-strip" style={{ "--columns": Math.min(models.length, 8) }}>{models.map((model, modelIndex) => { const result = currentCase && getResult(currentCase.id, model); const score = currentCase ? getScore(currentCase, model, cases.indexOf(currentCase), modelIndex) : null; const output = result?.text || (isPreview ? "这是合成示例输出。实际评测会在这里展示模型原始回答、评分依据、延迟与成本。" : evaluation?.status === "running" ? "等待模型返回…" : result?.error || "暂无输出"); return <article className="result-card" key={model.id}><header><div><ModelMark model={model} /><strong>{model.name}</strong></div><ScoreState score={score} /></header><p>{output}</p>{result?.error ? <div className="verdict risk"><XCircle />{result.error}</div> : <div className={`verdict ${score >= 8 ? "safe" : score >= 6.5 ? "review" : "risk"}`}>{score >= 8 ? <><ShieldCheck />通过</> : score >= 6.5 ? <><WarningCircle />建议复核</> : <><XCircle />未通过</>}</div>}<button className="card-action" disabled={!result} onClick={() => setReviewing({ result, model, score: result.assessment?.score ?? 0, verdict: "reviewed", notes: "" })}>{result ? "查看输出与人工复核" : isPreview ? "运行后可查看" : "等待结果"}</button></article>; })}</div></div>
      </section> : view === "grid" ? <section className="alternate-view"><div className="alternate-heading"><h2>全部模型结果</h2><span>布局随模型数量自动调整</span></div><div className={`output-grid count-${Math.min(models.length, 8)}`}>{models.map((model, modelIndex) => { const result = currentCase && getResult(currentCase.id, model); const score = currentCase ? getScore(currentCase, model, cases.indexOf(currentCase), modelIndex) : null; return <article className="large-output" key={model.id}><header><div><ModelMark model={model} /><div><strong>{model.name}</strong><span>{model.provider}</span></div></div><ScoreState score={score} /></header><p>{result?.text || (isPreview ? "合成示例：真实输出会完整保留在客户本地，并在此进行并排比较。" : "暂无输出")}</p><footer><span>延迟 {result ? `${(result.latencyMs / 1000).toFixed(2)}s` : "—"}</span><span>成本 {formatMoney(result?.cost)}</span><button disabled={!result} onClick={() => result && setReviewing({ result, model, score: result.assessment?.score ?? 0, verdict: "reviewed", notes: "" })}>人工复核</button></footer></article>; })}</div></section> : <section className="alternate-view"><div className="alternate-heading"><h2>聚焦对比</h2><span>从已完成的 {models.length} 个结果中选择两个精读</span></div><div className="model-chip-row">{models.map((model) => <button className={focusedModels.includes(model.id) ? "selected" : ""} key={model.id} onClick={() => setFocusedModels((current) => current.includes(model.id) ? current : [current.at(-1), model.id].filter(Boolean))}><ModelMark model={model} /><span>{model.name}<small>{Number.isFinite(summaries.get(model.id)?.score) ? `${summaries.get(model.id).score.toFixed(1)} 分` : "待评分"}</small></span></button>)}</div><div className="focus-pair">{focused.map((model, modelIndex) => { const result = currentCase && getResult(currentCase.id, model); const score = currentCase ? getScore(currentCase, model, cases.indexOf(currentCase), models.indexOf(model)) : null; return <article key={model.id}><header><div><ModelMark model={model} /><strong>{model.name}</strong></div><ScoreState score={score} /></header><p>{result?.text || (isPreview ? "合成示例用于确认双模型精读交互。完成真实评测后，此处展示原始输出和评分依据。" : "暂无输出")}</p><div className="assessment-reason"><strong>评分依据</strong><span>{result?.assessment?.reason || "等待评测结果"}</span></div></article>; })}</div></section>}
    </main>
    ...
    {showDataset && <Modal className="form-modal dataset-modal" onClose={() => setShowDataset(false)}><header><div><h2>导入评测数据集</h2><p>支持 JSONL、JSON 数组和简单 CSV；所有数据只保存在本地。</p></div><button className="close-button" onClick={() => setShowDataset(false)}><X /></button></header><form onSubmit={importDataset}><div className="form-grid"><label><span>数据集名称</span><input required value={datasetForm.name} onChange={(event) => setDatasetForm({ ...datasetForm, name: event.target.value })} placeholder="例如：客服安全测试集" /></label><label><span>当前数据集</span><select value={selectedDatasetId} onChange={(event) => setSelectedDatasetId(event.target.value)}>{state.datasets.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.cases.length} 条</option>)}</select></label><label className="full"><span>说明</span><input value={datasetForm.description} onChange={(event) => setDatasetForm({ ...datasetForm, description: event.target.value })} placeholder="数据来源和用途（请勿粘贴敏感数据）" /></label><label className="full file-label"><span>上传 CSV / JSONL</span><input type="file" accept=".csv,.json,.jsonl,text/csv,application/json" onChange={(event) => readDatasetFile(event.target.files?.[0])} /></label><label className="full"><span>数据内容</span><textarea className="dataset-text" required value={datasetForm.text} onChange={(event) => setDatasetForm({ ...datasetForm, text: event.target.value })} placeholder={'id,input,expected_keywords,tags\nTC-001,退款规则是什么,退款|规则,售后'} /></label></div><footer><span className="format-help">关键词与标签使用 | 分隔；CSV 中暂不支持带换行的字段。</span><button className="primary-button" disabled={busy === "dataset"}>{busy === "dataset" ? "导入中…" : "导入并选用"}</button></footer></form></Modal>}

    {showRun && <Modal className="form-modal run-modal" onClose={() => setShowRun(false)}><header><div><h2>创建模型评测</h2><p>{selectedModels.length} 个模型 × {state.datasets.find((item) => item.id === selectedDatasetId)?.cases.length || 0} 条用例</p></div><button className="close-button" onClick={() => setShowRun(false)}><X /></button></header><form onSubmit={createEvaluation}><div className="form-grid"><label><span>任务名称</span><input required value={runForm.name} onChange={(event) => setRunForm({ ...runForm, name: event.target.value })} /></label><label><span>数据集</span><select required value={selectedDatasetId} onChange={(event) => setSelectedDatasetId(event.target.value)}>{state.datasets.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.cases.length} 条</option>)}</select></label><label className="full"><span>System Prompt</span><textarea value={runForm.systemPrompt} onChange={(event) => setRunForm({ ...runForm, systemPrompt: event.target.value })} /></label><label><span>Temperature</span><input type="number" min="0" max="2" step="0.1" value={runForm.temperature} onChange={(event) => setRunForm({ ...runForm, temperature: event.target.value })} /></label><label><span>Max tokens</span><input type="number" min="1" max="32768" value={runForm.maxTokens} onChange={(event) => setRunForm({ ...runForm, maxTokens: event.target.value })} /></label><label><span>本地并发数</span><input type="number" min="1" max="12" value={runForm.concurrency} onChange={(event) => setRunForm({ ...runForm, concurrency: event.target.value })} /></label><label><span>超时</span><input value="60 秒" disabled /></label><label className="full"><span>评分规则</span><textarea value={runForm.rubric} onChange={(event) => setRunForm({ ...runForm, rubric: event.target.value })} /></label><fieldset className="full"><legend>可选：LLM-as-a-Judge</legend><label className="check-row"><input type="checkbox" checked={runForm.judgeEnabled} onChange={(event) => setRunForm({ ...runForm, judgeEnabled: event.target.checked })} />使用独立模型评分；失败时自动回退关键词规则</label>{runForm.judgeEnabled && <div className="inline-fields"><select required value={runForm.judgeConnectionId} onChange={(event) => setRunForm({ ...runForm, judgeConnectionId: event.target.value, judgeModel: state.connections.find((item) => item.id === event.target.value)?.models[0] || "" })}><option value="">选择 Judge 连接</option>{state.connections.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input required value={runForm.judgeModel} onChange={(event) => setRunForm({ ...runForm, judgeModel: event.target.value })} placeholder="Judge 模型 ID" /></div>}</fieldset><div className="full pricing-section"><div><strong>可选：每百万 Token 价格（USD）</strong><span>用于估算每个模型的调用成本</span></div>{selectedModels.map((id) => { const model = availableModels.find((item) => item.id === id); if (!model) return null; return <div className="pricing-row" key={id}><span>{model.name}</span><input type="number" min="0" step="0.0001" placeholder="输入价" value={runForm.pricing[id]?.input || ""} onChange={(event) => setRunForm({ ...runForm, pricing: { ...runForm.pricing, [id]: { ...runForm.pricing[id], input: event.target.value } } })} /><input type="number" min="0" step="0.0001" placeholder="输出价" value={runForm.pricing[id]?.output || ""} onChange={(event) => setRunForm({ ...runForm, pricing: { ...runForm.pricing, [id]: { ...runForm.pricing[id], output: event.target.value } } })} /></div>; })}</div></div><footer><span className="format-help">API 调用将在本机服务端执行，运行后可关闭窗口。</span><button className="primary-button" disabled={busy === "evaluation"}>{busy === "evaluation" ? "正在创建…" : "开始评测"}</button></footer></form></Modal>}
