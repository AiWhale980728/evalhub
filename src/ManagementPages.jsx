...
export function ManagementPages({
  ...
  function Overview() {
    ...
    return <><PageHeader eyebrow="工作台 / 概览" title="模型评测概览" description="从数据准备、模型接入到复核导出，快速掌握本地评测状态。"><button className="primary-button" onClick={openRun}><Plus />新建评测</button></PageHeader>
      {pristine ? <section className="onboarding-card"><div><span className="onboarding-mark"><ChartBar weight="fill" /></span><div><strong>这是一个全新的本地工作台</strong><p>先连接你自己的模型 API，再导入评测数据集。这里不会预置虚构的评分、历史任务或业务数据。</p></div></div><ol><li><b>1</b><span><strong>连接模型</strong><small>添加厂商 API 与模型型号</small></span></li><li><b>2</b><span><strong>导入数据</strong><small>CSV、JSON 或 JSONL</small></span></li><li><b>3</b><span><strong>开始评测</strong><small>同时比较 2–8 个模型</small></span></li></ol><footer><button className="primary-button" onClick={openApi}><Key />添加 API 连接</button><button className="secondary-button" onClick={openDatasetImport}><Database />导入数据集</button><button className="text-button" disabled={demoBusy} onClick={loadDemo}>{demoBusy ? "正在加载…" : "加载可删除的示例项目"}</button></footer></section> : null}
